...
struct SetWithNull {
  ...
  bool insert(const DecodedVector* arrayElements, vector_size_t offset) {
    return set.insert(arrayElements->template valueAt<T>(offset)).second;
  }

  bool count(const DecodedVector* arrayElements, vector_size_t offset) const {
    return set.count(arrayElements->template valueAt<T>(offset));
  }
...
template <>
struct SetWithNull<ComplexType> {
  ...
  folly::F14FastSet<TKey, Hash, EqualTo> set;
  bool hasNull{false};
  static constexpr vector_size_t kInitialSetSize{128};

  bool insert(const DecodedVector* arrayElements, vector_size_t offset) {
    const auto vector = arrayElements->base();
    const auto index = arrayElements->index(offset);
    const uint64_t hash = vector->hashValueAt(index);
    return set.insert(std::make_tuple(hash, vector, index)).second;
  }
  ...
  bool count(const DecodedVector* arrayElements, vector_size_t offset) const {
    ...
    const auto index = arrayElements->index(offset);
    const uint64_t hash = vector->hashValueAt(index);
    return set.count(std::make_tuple(hash, vector, index));
  }
...
std::shared_ptr<exec::VectorFunction> createArrayIntersect(
  ...
  return VELOX_DYNAMIC_TEMPLATES_TYPE_DISPATCH(
...
std::vector<std::shared_ptr<exec::FunctionSignature>> signatures(
  ...
  return std::vector<std::shared_ptr<exec::FunctionSignature>>{
      exec::FunctionSignatureBuilder()
          .typeVariable("T")
          .returnType(fmt::format(returnTypeTemplate.c_str()))
