...
class WriterFuzzer {
 ...
 private:
  ...
  fuzzer::ResultOrError execute(
      const core::PlanNodePtr& plan,
      int32_t maxDrivers = 2);

  // Compare if two directories have same partitions
  ...
  std::set<std::string> getPartitionNames(std::string outputDirectoryPath);
...
void WriterFuzzer::go() {
  ...
  while (!isDone(iteration, startTime)) {
    ...
    auto partitionOffset = names.size();
...
std::vector<std::string> WriterFuzzer::generatePartitionKeys(
    ...
    std::vector<std::string>& names,
    std::vector<TypePtr>& types) {
  // Supported partition key  columns types
  ...
  for (auto i = 0; i < numKeys; ++i) {
    ...
    types.push_back(vectorFuzzer_.randType(kNonFloatingPointTypes, 1));
    names.push_back(keys.back());
  }
  return keys;
}
...
std::vector<RowVectorPtr> WriterFuzzer::generateInputData(
  ...
  auto partitionCount = inputType->size() - partitionOffset;
...
void WriterFuzzer::verifyWriter(
  ...
  if (resultOrError.result == nullptr) {
    LOG(ERROR) << "Velox plan execution failed";
    return;
  }

  auto dropSql = "DROP TABLE IF EXISTS tmp_write";
  auto createSql = referenceQueryRunner_->toSql(plan);
  auto insertSql = "INSERT INTO tmp_write SELECT * FROM tmp";
  if (!createSql.has_value()) {
    LOG(INFO) << "Query not supported by the reference DB";
    return;
  }
  referenceQueryRunner_->execute(dropSql);
  referenceQueryRunner_->execute(createSql.value());
  std::multiset<std::vector<variant>> expectedResult;
  try {
    expectedResult =
        referenceQueryRunner_->execute(insertSql, input, plan->outputType());
  } catch (...) {
  ...
  // TableWrite node produces results of following layout
  // row     fragments     context
  // X         null          X
  // null       X            X
  // null       X            X
  // Extract inserted rows from velox execution result.
  std::vector<VectorPtr> insertedRows = {
      resultOrError.result->childAt(0)->slice(0, 1)};
  ...
  auto path = referenceQueryRunner_->execute("SELECT \"$path\" FROM tmp_write");
  auto tableDirectoryPath =
      fs::path(extractSingleValue<StringView>(path)).parent_path();
  for (int i = 0; i < partitionKeys.size(); i++) {
    tableDirectoryPath = tableDirectoryPath.parent_path();
  }
  VELOX_CHECK(
      comparePartitions(outputDirectoryPath, tableDirectoryPath),
      "Velox and reference DB output directory hierarchies don't match");

  LOG(INFO) << "Verified results against reference DB";
}

fuzzer::ResultOrError WriterFuzzer::execute(
    const core::PlanNodePtr& plan,
    int32_t maxDrivers) {
...
bool WriterFuzzer::comparePartitions(
    const std::string& outputDirectoryPath,
    const std::string& referenceOutputDirectoryPath) {
  auto partitions = getPartitionNames(outputDirectoryPath);
  auto referencedPartitions = getPartitionNames(referenceOutputDirectoryPath);
  ...
  return partitions == referencedPartitions;
}
